    ...
    <div className={overviewStyles.overviewContainer}>
      ...
      <div className={overviewStyles.statsGrid}>
        <div className={overviewStyles.statCard}>
          ...
          <div className={overviewStyles.statCard} onClick={() => setShowExercisesModal(true)} style={{ cursor: 'pointer' }}>
          <div className={overviewStyles.statIcon}>
            <FiCode />
          </div>
          <div className={overviewStyles.statContent}>
        ...
        <div className={overviewStyles.calendarGrid}>
          {userStats.calendarData?.dailyActivities?.map((day: any, i: number) => {
            ...
            return (
              ...
              >
                  <div className={`${overviewStyles.dayName} ${hasActivity ? overviewStyles.hasActivity : ''}`}>{dayName}</div>
      ...
      {showExercisesModal && (
        <div className={overviewStyles.modalOverlay} onClick={() => setShowExercisesModal(false)}>
          <div className={overviewStyles.modalContent} onClick={(e) => e.stopPropagation()}>
              ...
              {solvedExercisesData && solvedExercisesData.length > 0 ? (
                <div className={overviewStyles.exercisesList}>
                  {solvedExercisesData.map((exercise: any, index: number) => (
                    <div key={exercise.id || index} className={overviewStyles.exerciseItem}>
                      <div className={overviewStyles.exerciseHeader}>
                        <h3 className={overviewStyles.exerciseTitle}>{exercise.title}</h3>
                        <span className={`${overviewStyles.difficultyBadge} ${overviewStyles[exercise.difficulty?.toLowerCase()]}`}>
                          {exercise.difficulty}
                        </span>
                      </div>
                      <div className={overviewStyles.exerciseStats}>
                        <span className={overviewStyles.exerciseStat}>
                          <FiClock /> {exercise.content?.timeComplexity || 'O(1)'}
                        </span>
                        <span className={overviewStyles.exerciseStat}>
                          <FiActivity /> {exercise.content?.spaceComplexity || 'O(1)'}
                        </span>
                        <span className={overviewStyles.exerciseStat}>
                          <FiCheckCircle /> {exercise.content?.acceptance || 0}% qəbul
                        </span>
                      </div>
                    </div>
                  ))}
